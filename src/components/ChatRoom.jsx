...
const socket = io(window.location.origin.replace(/^http/, 'ws').replace(/:\d+$/, ':3001'))
...
export default function ChatRoom() {
  ...
  const joinChat = () => { if (!nick) return; socket.emit('setNick', nick, (res) => res.success ? setConnected(true) : alert(res.message)) }
  const sendMessage = () => { if (msg.trim() === '') return; socket.emit('chatMessage', msg); setMsg('') }
      ...
      {!connected ? (
        <div className="flex flex-col items-center gap-4">
          <input
            ...
            className="px-4 py-3 rounded-2xl w-full text-black focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          <button
            onClick={joinChat}
